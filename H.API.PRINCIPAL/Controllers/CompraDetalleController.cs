...
namespace H.API.PRINCIPAL.Controllers
{
    ...
    public class CompraDetalleController : ControllerBase
    {
        ...
        public CompraDetalleController(IUnitOfWork unitOfWork)
        {
            this.unitOfWork = unitOfWork;
        }

        [HttpPost("Insertar")]
        public IActionResult Insert([FromBody] CompraDetalle categoria)
        {
            try
            {
                var servicio = new CompraDetalleService(unitOfWork);
        ...
        [HttpPut("Modificar")]
        public IActionResult Update([FromBody] CompraDetalle categoria)
        {
            try
            {
                var servicio = new CompraDetalleService(unitOfWork);
        ...
        public IActionResult InsertarMultipleTabla(CompraInsertDTO categoria)
        {
            try
            {
                var servicio = new CompraDetalleService(unitOfWork);
                var respuesta = servicio.AddMultipleTabla(categoria);
                return Ok(respuesta);
            }
            catch (Exception ex)
            {
