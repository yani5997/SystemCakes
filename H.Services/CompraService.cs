...
namespace H.Services
{
    public class CompraService: ICompraService
    {
        ...
        public int Add(Compra entidad)
        {
            try
            {
                var modelo = _unitOfWork.CompraRepository.Add(entidad);
                _unitOfWork.Commit();
                return modelo.Id;
            }
            catch (Exception ex)
        ...
        public int Update(Compra entidad)
        {
            try
            {
                var modelo = _unitOfWork.CompraRepository.Update(entidad);
                _unitOfWork.Commit();
                return modelo.Id;
            }
            catch (Exception ex)
            {
                var error = new Error();
                error.Message = "CompraService" + ex.Message;
                error.Exception = ex;
        ...
        public int Delete(int id, string usuario)
        {
            try
            {
                var rpta = _unitOfWork.CompraRepository.Delete(id, usuario);
                _unitOfWork.Commit();
                return rpta;
            }
            catch (Exception ex)
        ...
        public Compra GetById(int id)
        {
            try
            {
                return _unitOfWork.CompraRepository.GetById(id);
            }
            catch (Exception ex)
            {
                var error = new Error();
        ...
        public IEnumerable<CompraListadoDTO> ObtenerCombo()
		{
			try
			{
				return _unitOfWork.CompraRepository.ObtenerCombo();
			}
			catch (Exception ex)
			{
				var error = new Error();
